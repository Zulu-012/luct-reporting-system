...
import { fetchLectures, exportToCSV, getClasses, getCourses } from "../api";
...
export default function ReportsList({ user }) {
  ...
  const loadAllData = async () => {
    ...
    setMessage("");
    try {
      const [lecturesData, classesData, coursesData] = await Promise.all([
        fetchLectures(),
        getClasses(),
        getCourses()
      ]);
