  ...
  const loadLecturesAndRatings = useCallback(async () => {
    try {
      ...
      if (user.role === "student") {
        ...
        // Filter to only show past lectures for rating
        lecturesData = allLectures.filter(lecture =>
        ...
        // Program leaders see lectures from their courses
        const programCourses = await getCoursesByProgramLeader(user.id);
        const courseIds = programCourses.map(c => c.id);
        lecturesData = lecturesData.filter(lec => courseIds.includes(lec.course_id));
      }
  ...
  return (
    <div className="rating-container">
      ...
      <div className="rating-content">
        ...
        {/* My Lecture Ratings Tab (For Lecturers and Other Roles - View Only) */}
        {activeTab === "myratings" && user.role !== "student" && (
          <div className="tab-content">
            <div className="section-header">
              <h2>My Lecture Ratings</h2>
              <p>View ratings and feedback for your lectures</p>
            </div>
                  ...
                  <div key={lecture.id} className="lecture-card rating-card">
                    ...
                    <div className="card-actions">
                      <div className="feedback-summary">
                        <strong>Your Feedback:</strong> {lectureFeedbacks.length} comments given
                      </div>
                      <button 
            ...
            {lectures.length === 0 && (
              <div className="no-lectures">
                ...
                <p>You have no lectures assigned at the moment.</p>
              </div>
            )}
          </div>
        )}

        {/* Give Feedback Tab (For Lecturers) */}
          ...
          <div className="tab-content">
            ...
            {selectedLecture ? (
              ...
              <div className="select-item-prompt">
                ...
                <p>Choose a lecture from the "My Lecture Ratings" tab to provide feedback to students</p>
              </div>
            )}
          </div>
        )}
