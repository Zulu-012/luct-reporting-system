...
// === FEEDBACK ROUTES ===
// POST feedback on a lecture - Only principal lecturers can give feedback
app.post('/feedback', authenticateToken, requireRole(['principal_lecturer']), async (req, res) => {
  ...
  try {
    const sql = `INSERT INTO feedback (lecture_id, principal_lecturer_id, feedback_text) VALUES (?, ?, ?)`;
...
app.get('/feedback/:lecture_id', authenticateToken, requireRole(['admin', 'principal_lecturer', 'lecturer']), async (req, res) => {
...
app.get('/rating/:lecture_id', async (req, res) => {
  const { lecture_id } = req.params;
  try {
    const sql = `
    ...
    `;
    const [rows] = await pool.execute(sql, [lecture_id]);
    res.json(rows[0] || { average_rating: null, total_ratings: 0 });
  } catch (err) {
...
// GET user's rating for a lecture
app.get('/rating/:lecture_id/user', authenticateToken, async (req, res) => {
  const { lecture_id } = req.params;
  try {
    const sql = `
      SELECT rating 
